...
// function select2_dinamis($name,$table,$field,$placeholder,$pk,$selected=null,$order=null){
//     $ci = get_instance();
//     $select2 = '<select name="'.$name.'" class="form-control select2 select2-hidden-accessible" multiple="" 
//                data-placeholder="'.$placeholder.'" style="width: 100%;" tabindex="-1" aria-hidden="true">';
//     $data = $ci->db->get($table)->result();
...
function datalist_dinamis($name,$table,$field,$value=null,$selected=null){ 
    $ci = get_instance();
    $string = '<input value="'.$value.'" name="'.$name.'" list="'.$name.'" class="form-control">
    <datalist id="'.$name.'">';
    $data = $ci->db->get($table)->result();
    foreach ($data as $row){
        // $string .="<option value='".$row->id_produk."'";
        // $string .= $selected==$row->$field?" selected='selected'":'';
        // $string .=">".  strtoupper($row->$field)."</option>";

        $string.='<option value="'.$row->$field.'">';
        $string.= '<option value="'.$row->$name.'" "selected">'.$row->nama_produk.'</option>';
        // echo "<option value='$p->id' 'selected'>$p->name_pro</option>";
